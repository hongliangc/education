...
- **`dump-pinyin-audio-items.mjs`** — 从汉字模块的拼音表生成 AWS Polly 音频清单 `scripts/pinyin-audio-items.json`；运行时需带 `scripts/ts-resolve-hooks.mjs` loader。
- **`gen-pinyin-audio.py`** — 使用 AWS Polly 东京区 `Zhiyu/neural` 离线生成全部拼音基础音与有效的常用声调例字音频到 `public/audio/pinyin/`；声母不生成四声，缺少低龄常用字的声调不强行补生僻字。
