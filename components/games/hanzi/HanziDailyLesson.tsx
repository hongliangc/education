...
export function HanziDailyLesson({
  ...
  return (
    <section className="flex h-[min(94vh,64rem)] flex-col gap-4 overflow-y-auto bg-[#fffdf9] p-4 sm:p-6">
      <HanziScreenHeader title="逐字学习" subtitle="我会数一数" onBack={leave} progress={`${index + 1}/${items.length}`} />
      <div className="grid min-h-0 flex-1 gap-4 md:grid-cols-[12rem_minmax(0,1fr)]">
        <aside className="rounded-3xl bg-sky-50 p-3 ring-1 ring-sky-100"><div className="text-center font-black text-sky-700">本单元汉字</div><nav aria-label="本单元汉字" className="mt-3 flex gap-2 overflow-x-auto md:grid md:grid-cols-2 md:content-start md:overflow-y-auto">
          {items.map((navItem, navIndex) => <button key={navItem.id} type="button" onClick={() => { stopSpeech(); setIndex(navIndex); setExplained(false); }} className={`min-h-12 min-w-16 rounded-2xl px-4 text-2xl font-black ring-1 ${navIndex === index ? "bg-sky-500 text-white ring-sky-600" : "bg-white text-slate-600 ring-slate-200"}`}>{navItem.char}</button>)}
        </nav><div className="mt-3 text-center text-xs font-black text-slate-500">⭐ 学习顺序：{index + 1} / {items.length}</div></aside>
        <div className="flex min-h-0 flex-col gap-4">
          <div className="grid items-center gap-5 rounded-[2rem] bg-gradient-to-br from-amber-50 to-orange-100 p-5 ring-1 ring-orange-100 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-3xl bg-white/80 p-4 text-left ring-1 ring-orange-100 sm:col-span-2 lg:col-span-1"><div className="font-black text-orange-700">💡 记忆提示</div><p className="mt-2 text-sm font-bold leading-7 text-slate-600">{item.story}</p></div>
          </div>
