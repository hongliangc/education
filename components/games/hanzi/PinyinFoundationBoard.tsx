...
import { PINYIN_CHART, PINYIN_FOUNDATIONS, pinyinSsml, syllableToneSsml, toneMarkedSyllables, type PinyinCategory, type PinyinChartItem } from "@/content/hanzi";
...
const INITIAL_TONE_BASE: Record<string, string> = { b: "ba", p: "pa", m: "ma", f: "fa", d: "da", t: "ta", n: "na", l: "la", g: "ga", k: "ka", h: "ha", j: "ji", q: "qi", x: "xi", zh: "zha", ch: "cha", sh: "sha", r: "re", z: "za", c: "ca", s: "sa", y: "yi", w: "wu" };
...
export function PinyinFoundationBoard({
  ...
  const toneBase = selected?.category === "initial" ? INITIAL_TONE_BASE[selected.display] : selected?.display;
  const toneChoices = selected && toneBase ? toneMarkedSyllables(toneBase).map((label, index) => ({
    label,
    ssml: toneItem ? pinyinSsml(toneItem.ssmlBase, (index + 1) as 1 | 2 | 3 | 4) : syllableToneSsml(selected.category === "initial" ? toneBase : selected.phonemeBase, (index + 1) as 1 | 2 | 3 | 4),
  })) : [];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 overflow-x-auto px-2 pb-2 scroll-hide">
        ...
        <Btn className="min-h-11 shrink-0 px-6" variant="secondary" onClick={() => speakPinyin(sequenceSsml(items), items.map((item) => item.fallback).join("，"))}>🔊 全部朗读</Btn>
      </div>
      <div className="grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        {items.map((item) => <button key={item.id} type="button" onClick={() => { setSelected(item); speakPinyin(item.lessonSsml, `${item.mouthHint}。${item.fallback}，${item.fallback}`); }} className={`min-h-20 rounded-2xl px-3 py-3 text-2xl font-black ring-2 ${selected?.id === item.id ? "bg-sky-500 text-white ring-sky-600" : "bg-white text-slate-700 ring-slate-200"}`}>{item.display}</button>)}
      ...
      {selected ? <div className="rounded-[2rem] bg-gradient-to-br from-sky-50 to-purple-50 p-5 text-center ring-1 ring-sky-100">
        ...
        {toneChoices.length > 0 ? <div className="mt-4"><div className="text-sm font-black text-pink-600">{selected.category === "initial" ? "拼读四声" : "四声"}</div><div className="mt-2 grid grid-cols-4 gap-2">{toneChoices.map((tone, index) => <button key={tone.label} type="button" onClick={() => speakPinyin(tone.ssml, toneNarration(tone.label, index))} className="rounded-xl bg-pink-50 py-2 text-xl font-black text-pink-600 ring-1 ring-pink-100">{tone.label}</button>)}</div></div> : null}
...
function toneNarration(tone: string, index: number): string {
  const names = ["第一声，平平的", "第二声，向上扬", "第三声，先降再升", "第四声，向下降"] as const;
  return `${tone}，${names[index] ?? names[0]}`;
}
