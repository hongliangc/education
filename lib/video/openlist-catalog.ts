...
function compareEpisodes(aTitle: string, bTitle: string): number {
...
export function buildVideoCatalog(
  ...
  sortedCategories.forEach((cat, categoryIndex) => {
    ...
    const hasNumberedEpisodes = mediaFiles.some((name) => episodeNumber(cleanTitle(name)) !== null);
    const orderedMediaFiles = hasNumberedEpisodes
      ? [...mediaFiles].sort((a, b) => compareEpisodes(cleanTitle(a), cleanTitle(b)))
      : mediaFiles;

    orderedMediaFiles.forEach((name, index) => {
